...
// Parse one light object  { ... }
// Returns false when the source string runs out of objects.
static bool parseLight(const char*& p, Light& out)
{
    ...
    while (*p && *p != '}')
    {
        ...
        if      (key == "name")        out.name        = readString(p);
        else if (key == "type")        out.type        = (LightType)(int)readFloat(p);
        else if (key == "enabled")     out.enabled     = readBool(p);
        else if (key == "color")       out.color       = readVec3(p);
        else if (key == "intensity")   out.intensity   = readFloat(p);
        else if (key == "position")    out.position    = readVec3(p);
        else if (key == "direction")   out.direction   = readVec3(p);
        else if (key == "constant")    out.constant    = readFloat(p);
        else if (key == "linear")      out.linear      = readFloat(p);
        else if (key == "quadratic")   out.quadratic   = readFloat(p);
        else if (key == "innerCutoff") out.innerCutoff = readFloat(p);
        else if (key == "outerCutoff") out.outerCutoff = readFloat(p);
        else
        {
    ...
    : m_jsonPath(jsonPath)
{
    ...
    if (lights.empty())
    {
        ...
        save();   // Create lights.json immediately
    }
}
...
{
    if (index < 0 || index >= (int)lights.size()) return;
    if ((int)lights.size() >= MAX_LIGHTS) return;
...
// ── uploadToShader ──────────────────────────────────────────────────────────
// Uploads all enabled lights into the uniform arrays expected by the GLSL
// snippet in model_fragment_lights.glsl (see that file for exact names).
...
void LightManager::uploadToShader(Shader& shader) const
{
    ...
    for (const auto& l : lights)
    {
        ...
        // Convert degrees → cosines for the GLSL dot-product test
        shader.setFloat(base + "innerCutoff",
                        std::cos(glm::radians(l.innerCutoff)));
        shader.setFloat(base + "outerCutoff",
                        std::cos(glm::radians(l.outerCutoff)));
        ++n;
    }
