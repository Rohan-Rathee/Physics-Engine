...
void Animator::UpdateAnimation(float deltaTime)
{
    ...
    if (blendLayers.size() == 1)
    {
        ...
        CalculateBoneTransform(
            &blendLayers[0].animation->GetRootNode(),
            glm::mat4(1.0f),
            blendLayers[0].animation,
            blendLayers[0].time
        );
    }
    else
...
void Animator::CalculateBlendedBoneTransforms()
{
    ...
    std::vector<BoneAccum> accum(boneCount);
    ...
    for (const auto& layer : blendLayers)
    {
        ...
        for (const auto& [boneName, boneInfo] : boneInfoMap)
        {
            ...
            DecomposedTransform dt = DecomposeMatrix(globalTransforms[index]);

            BoneAccum& ba = accum[index];
            if (!ba.hasValue)
            {
    ...
    for (const auto& [boneName, boneInfo] : boneInfoMap)
    {
        ...
        if (!accum[index].hasValue)
            continue;

        glm::mat4 blendedGlobal = RecomposeMatrix({
            accum[index].translation,
            accum[index].rotation,
            accum[index].scale
        });
