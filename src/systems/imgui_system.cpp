...

{
...

    {
        ...
        if (ImGui::MenuItem("Point Light"))       doAdd(LightType::Point,       "Point Light");

        if (ImGui::MenuItem("Directional Light")) doAdd(LightType::Directional, "Directional Light");

        if (ImGui::MenuItem("Spot Light"))        doAdd(LightType::Spot,        "Spot Light");
    ...
    if (dupDisabled) ImGui::BeginDisabled();
    ...
    if (dupDisabled) ImGui::EndDisabled();
...

    {
...

        {
...

            {
                ...
                float sq = l.linear * l.linear

                         - 4.0f * l.quadratic * (l.constant - thresh);
...

{
    ...
    ImGui::SetNextWindowSize(ImVec2(barW + textW + pad * 2.0f,
                                    ...
                                    barH + pad * 2.0f), ImGuiCond_Always);
