...
bool RenderSystem::isInFrustum(const glm::vec3& pos) {
    for (int i = 0; i < 6; i++) {
        ...
        if (p.x*pos.x + p.y*pos.y + p.z*pos.z + p.w <= -1.0f)
            return false;
    }
    return true;
}
...
void RenderSystem::render(float currentFrame, const glm::mat4& view, const glm::mat4& projection) {
    ...
    lightPos = glm::vec3(50.0f * cos(currentFrame), 50.0f, 50.0f * sin(currentFrame));
    glm::vec3 lightTarget = glm::vec3(0.0f, 0.0f, 0.0f);
    lightView = glm::lookAt(lightPos, lightTarget, glm::vec3(0.0f, 1.0f, 0.0f));
    lightProjection = glm::ortho(-100.0f, 100.0f, -100.0f, 100.0f, 0.1f, 500.0f);
