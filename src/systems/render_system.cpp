...
const float cubeVertices[] = {
    // Back face (z = -0.5) — FIXED (reversed)
...
{
    // modelTransform will be set later by Engine via setModelTransformPtr
}
...
void RenderSystem::setupModels()
{
    ...
    // Create 10x10 grid of basketballs - build collision shape from first, reuse for all
    ...
    for (int i = 0; i < 1; ++i){
        for (int j = 0; j < 1; ++j){
            ...
            if (modelTransform && basketballShape)
            {
                ...
                //index, mass(in kg), shape, restitution
            }
        }
    }


}// For testing: add a single cube model with a box collider
...
void RenderSystem::RenderPass(const Camera &camera, const glm::mat4 &view, const glm::mat4 &projection, glm::mat4 lightSpaceMatrix, float currentFrame)
{
    ...
    if (modelShader)
    {
        ...
        glBindTexture(GL_TEXTURE_2D, shadowDepthMap);// Bind the shadow map to texture unit 1
        ...
        if (!modelLoader->models.empty())
        {
          ...
          //     std::cerr << "No models to render!" << std::endl;
    }

    // Draw Bullet physics debug shapes (collision wireframes)
