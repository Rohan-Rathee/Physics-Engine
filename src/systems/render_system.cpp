...
void RenderSystem::setupModels() {
    ...
    // for importing from blender, we need to rotate the model 90 degrees around the x-axis to align it with OpenGL's coordinate system
    setModelTransform(0, glm::vec3(+20.0f, -10.0f, +20.0f), glm::vec3(1.0f), 90.0f, glm::vec3(-1.0f, 0.0f, 0.0f));
    //id, vertexPath, fragmentPath, position, scale, rotationAngle, rotationAxis
    modelShader->use();
}
